...
public class AvailableOffersOutboxDao {
    ...
    private final PreparedStatement selectNew;
    ...
    public AvailableOffersOutboxDao(CqlSession session) {
        ...
        this.selectNew = session.prepare(
        ...
        this.insertRecord = session.prepare(
        ...
        this.deleteRecord = session.prepare(
    ...
    public List<AvailableOffersOutboxRecord> fetch(String status, int bucket, int limit) {
        BoundStatement stmt = selectNew.bind(status, bucket).setPageSize(limit);
    ...
    public void insert(AvailableOffersOutboxRecord record) {
        ...
        BoundStatement stmt = insertRecord.bind(
                k.getStatus(),
                k.getBucket(),
                k.getCreatedAt(),
                k.getEventId(),
    ...
    public void delete(AvailableOffersOutboxKey key) {
        session.execute(deleteRecord.bind(key.getStatus(), key.getBucket(), key.getCreatedAt(), key.getEventId()));
    }

    public void updateAttemptsAndError(AvailableOffersOutboxKey key, int attempts, String lastError) {
        session.execute(updateAttemptsError.bind(attempts, lastError, key.getStatus(), key.getBucket(), key.getCreatedAt(), key.getEventId()));
    }
